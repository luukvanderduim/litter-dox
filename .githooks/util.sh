...
function check_tests() {
    hook_info "🧪 Running 'cargo test'"
    cargo test \
        && hook_success "Tests passed" \
        || hook_failure "Cargo test detected errors."
}
...
function check_todos() {
    hook_info "📝 Checking for TODOs/FIXMEs"
    # Search for TODO or FIXME using a regex trick [T]ODO to avoid self-matching this script
    if grep -rE "[T]ODO|[F]IXME" . \
        --exclude-dir=.git \
        --exclude-dir=target \
        --exclude-dir=.githooks \
        --exclude-dir=.cargo \
        &> /dev/null; then
        hook_failure "Found TODOs or FIXMEs. Please address them before committing."
    else
        hook_success "No TODOs found."
    fi
}
